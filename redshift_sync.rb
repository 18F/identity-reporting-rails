...
# Initialize Rails environment for access to configuration and logging
require_relative 'config/environment'

# Redshift User Sync - Manages Redshift users, groups, and permissions
class RedshiftSync
  attr_reader :config_file_path, :users_yaml_path, :logger

  def initialize(config_file_path:, users_yaml_path:, logger: Rails.logger)
  ...
  def sync
    log_info('Starting Redshift user sync')
    ...
    log_info('Redshift user sync completed successfully')
  end
  ...
  def drop_users
    log_info('Dropping removed users')

    schemas = get_all_configured_schemas

    user_sql = users_to_drop(canonical_users, current_users).filter_map do |name|
      next if disallowed_characters?(name)

      log_info("Removing user #{name}")

      revoke_statements = schemas.map do |schema|
        <<~SQL
          REVOKE ALL ON SCHEMA #{schema} FROM "#{name}";
          REVOKE ALL ON ALL TABLES IN SCHEMA #{schema} FROM "#{name}";
        SQL
      end.join("\n")

      <<~SQL
        REVOKE ALL ON DATABASE analytics FROM "#{name}";
        #{revoke_statements}
        DROP USER "#{name}";
      SQL
    end
  ...
  def create_schema_privileges_for_group(user_group)
    ...
    revoke_statements = get_all_configured_schemas.map do |schema_name|
  ...
  def sync_user_group(group)
    log_info("Syncing users for #{group['name']}")
    ...
    if user_group_sql.any?
      ...
      log_info("User group #{group['name']} is empty")
    end
  end
  ...
  def log_info(message)
    logger.info({ name: 'RedshiftSync', message: message }.to_json)
  end

  def log_error(message)
    logger.error({ name: 'RedshiftSync', message: message }.to_json)
  end
end
