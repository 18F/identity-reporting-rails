  ...
  class PartnerReportDefaultMonthly
    ...
    def format_row_as_json(row)
...

      {
        ...
        provider_information: {
          ...
          start_service_provider_id: row['start_service_provider_id'],
        },
    ...
    def bulk_query
      <<~SQL
        ...
        FROM marts.service_providers sp_ref
        LEFT JOIN marts.sp_usage_metrics_monthly usage_data
            ON usage_data.service_provider_id = sp_ref.service_provider_id
            AND usage_data.period_date_id >= #{start_calendar_id}
            AND usage_data.period_date_id <= #{end_calendar_id}
        LEFT JOIN marts.sp_idv_outcomes_monthly sp_data
            ON sp_data.start_service_provider_id = sp_ref.service_provider_id
            AND sp_data.month_start_calendar_id >= #{start_calendar_id}
            AND sp_data.month_start_calendar_id <= #{end_calendar_id}
        LEFT JOIN marts.sp_auth_metrics_monthly auth_data
            ON auth_data.service_provider_id = sp_ref.service_provider_id
            AND auth_data.period_date_id >= #{start_calendar_id}
            AND auth_data.period_date_id <= #{end_calendar_id}
        LEFT JOIN marts.sp_account_creation_metrics_monthly acct_data
            ON acct_data.service_provider_id = sp_ref.service_provider_id
            AND acct_data.period_date_id >= #{start_calendar_id}
            AND acct_data.period_date_id <= #{end_calendar_id}
    ...
    def should_exclude_issuer?(issuer)
      ...
      # No filters specified - include all issuers
      false
    end

    def start_calendar_id
      time_range.begin.strftime('%Y%m01')  # First day of month
    end

    def end_calendar_id
      time_range.end.strftime('%Y%m01')    # First day of end month
    end

    def issuer_filter_clause
      if included_issuers&.any?
        sanitized = included_issuers.map { |i| ActiveRecord::Base.connection.quote(i) }
        "AND sp_ref.issuer IN (#{sanitized.join(', ')})"
      elsif excluded_issuers.any?
        sanitized = excluded_issuers.map { |i| ActiveRecord::Base.connection.quote(i) }
        "AND sp_ref.issuer NOT IN (#{sanitized.join(', ')})"
      else
        ''
      end
    end
