...
RSpec.describe AttemptsApiImportJob, type: :job do
  ...
  describe '#perform' do
    context 'when mock api returns an encrypted event' do
      ...
      #   allow(DataWarehouseApplicationRecord.connection).to receive(:execute).and_raise(ActiveRecord::StatementInvalid.new('DB error'))
      #   expect(Rails.logger).to receive(:info).with('AttemptsApiImportJob: Job started')
      #   expect(Rails.logger).to receive(:info).with(
      #     {
      #       job: 'AttemptsApiImportJob',
      #       success: false,
      #       message: 'AttemptsApiImportJob: Data import to Redshift failed',
      #       error: 'DB error',
      #     },
      #   )
      #   allow(DataWarehouseApplicationRecord.connection).to receive(:execute).and_raise(ActiveRecord::StatementInvalid.new('DB error'))
