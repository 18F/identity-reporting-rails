...
RSpec.describe FraudOpsEmailAddressesZeroEtlJob, type: :job do
  ...
  describe '#perform' do
    ...
    context 'when idp_zero_etl_enabled is true' do
      ...
      # The job never resolves the lookback itself, so the only way it can reach
      # the log is by way of the service's return value.
      it 'reports the lookback the service used' do
        allow(sync).to receive(:sync).and_return(sync_result.merge(lookback_minutes: 45))

        job.perform

        expect(Rails.logger).to have_received(:info).with(
          a_string_matching(/Job completed/).and(a_string_matching(/"lookback_minutes":45/)),
        )
      end
    end
